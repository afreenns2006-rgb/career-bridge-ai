...
def sync_language_state() -> None:
    """Keep widget state separate from app language state."""
...
def render_deployment_diagnostics() -> None:
    """Show deployment health without depending on local-only services."""
    ...
    try:
        db = get_db_manager()
        checks.append(("Database", "Available" if getattr(db, "available", False) else "Unavailable - session fallbacks active"))
    except Exception as exc:
        logger.warning("Diagnostics database check failed: %s", exc)
        checks.append(("Database", "Unavailable - session fallbacks active"))

    parser_status = []
    for package_name in ["pypdf", "PyPDF2", "pdfplumber", "docx"]:
        try:
            __import__(package_name)
            parser_status.append(f"{package_name}: ok")
        except Exception:
            parser_status.append(f"{package_name}: missing")
    checks.append(("File upload parsers", ", ".join(parser_status)))
    ...
    st.info("If a cloud service is missing, Career Bridge AI continues with local datasets and rule-based fallbacks.")
...
def render_career_mentor() -> None:
    ...
    with col2:
        ...
        interests = st.text_input(t("interests"), placeholder="AI, web development, data")
...
def main() -> None:
    ...
    # Route to selected page with a deployment-safe error boundary.
    try:
        if page == "Home":
            render_home_page()
